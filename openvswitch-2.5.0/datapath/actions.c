...
static int encap_libera(struct sk_buff *skb, struct sw_flow_key *key, struct ovs_action_encap_libera *libera){
   ...
   if(skb != NULL && key != NULL && libera != NULL) 
   {
       org_eth = eth_hdr(skb);
       ...
       if(skb_cow_head(skb, sizeof(struct udphdr)) < 0)
       {
           return 1;
       }
	...
	//ip
       if(iih != NULL) {
           if(skb_cow_head(skb, sizeof(struct iphdr)) < 0)
           {
               return 1;
           }
	...
	//eth
       if(skb_cow_head(skb, sizeof(struct ethhdr)) < 0)
       {
           return 1;
       }
