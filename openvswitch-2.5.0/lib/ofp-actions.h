...
#define OFPACTS                                                         \
    ...
    OFPACT(LIBERA_ENCAP,    ofpact_libera_encap,ofpact, "libera_encap")     \
    OFPACT(LIBERA_DECAP,    ofpact_libera_encap,ofpact, "libera_decap")     \
                                                                        \
    /* Metadata. */                                                     \
...
// --> ksyang
struct ofpact_libera_encap {
    struct ofpact ofpact;
    struct eth_addr src_mac;
    struct eth_addr dest_mac;
    ovs_be32 src_ipv4;
    ovs_be32 dest_ipv4;
    uint16_t src_port;              /* TCP, UDP or SCTP port number. */
    uint16_t dest_port;              /* TCP, UDP or SCTP port number. */
    ovs_be32 pad;
};

struct ofpact_libera_decap {
    struct ofpact ofpact;
    ovs_be32 pad;
};
